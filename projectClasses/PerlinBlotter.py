...
class PerlinBlotter:
    ...
    def blot(self, blot_sizeX, blot_sizeY):
        ...
        for x in range(0, blot_sizeX):
            for y in range(0, blot_sizeY):
                noiseWaarde = self.noiseMachine.waarde(x / self.scaleX,
                                                       y / self.scaleY,
    ...
    def blotVierkantRingen(self, blot_sizeX, blot_sizeY, doel_aantal_punten):
        ...
        min_grens = 0.0
        for i in range(0, 8):
            telling = blot_sizeX * blot_sizeY - np.count_nonzero(noiseWaardes < -self.grenswaarde) - np.count_nonzero(
                noiseWaardes > self.grenswaarde)
            if telling > doel_aantal_punten * 1.1:
                max_grens = self.grenswaarde
                self.grenswaarde = (min_grens + max_grens) * 0.5
            elif telling < doel_aantal_punten * 0.9:
    ...
    def line_blot(self, blot_sizeX, blot_sizeY, dikte, richtingGenerator):
        ...
        for x in range(0, blot_sizeX):
            for y in range(0, blot_sizeY):
                noiseWaardePos = self.noiseMachine.waarde(x / self.scaleX,
                                               y / self.scaleY,
                                               octaves=self.octaves,
                                               persistence=self.persistence,
                                               lacunarity=self.lacunarity,
                ...
                noiseWaardeNeg = self.noiseMachine.waarde((x + deltaX) / self.scaleX,
